...
function Template1({ cvData }) {
  ...
  return (
    <div className="template1-root" style={{ fontFamily }}>
      <div className="template1-layout">
        {/* LEFT SIDEBAR */}
        <div className="template1-sidebar" style={{ backgroundColor: theme }}>
          <div className="t1-photo-box text-center mb-3">
            <img src={profileImg} alt={personal.name} className="img-fluid rounded-circle" style={{ width: '130px', height: '130px', objectFit: 'cover', border: '3px solid white' }} />
          </div>

          {skills && skills.length > 0 && (
            <div className="t1-sidebar-section mb-3">
              <h4 className="text-white border-bottom pb-1"><i className="fas fa-laptop-code me-2"></i>Skills</h4>
              {skills.map((sk, idx) => (
          ...
          {socials && socials.length > 0 && (
            <div className="t1-sidebar-section mb-3">
              <h4 className="text-white border-bottom pb-1"><i className="fa fa-globe me-2"></i>Socials</h4>
              {socials.map((soc, idx) => (
                <div key={idx} className="small mb-1 text-white-50">
        ...
        {/* RIGHT MAINBAR */}
        <div className="template1-mainbar p-3">
          <div className="t1-header mb-3 pb-2 border-bottom" style={{ borderColor: theme }}>
            ...
            <div className="small text-muted mt-2 d-flex flex-wrap gap-3">
              ...
              {personal.email && <span><i className="fa fa-inbox me-1"></i><a href={`mailto:${personal.email}`} className="text-muted text-decoration-none">{personal.email}</a></span>}
              {personal.phone && <span><i className="fa fa-phone me-1"></i><a href={`tel:${personal.phone}`} className="text-muted text-decoration-none">{personal.phone}</a></span>}
              {personal.altPhone && <span><i className="fa fa-phone me-1"></i><a href={`tel:${personal.altPhone}`} className="text-muted text-decoration-none">{personal.altPhone}</a></span>}
            </div>
            {personal.industries && <div className="small text-secondary mt-1">{personal.industries}</div>}
          </div>

          {/* ADDITIONAL INFO */}
          {additional && (
            <div className="t1-section mb-3">
              ...
              <ul className="list-unstyled small mb-0 ms-2">
